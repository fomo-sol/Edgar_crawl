...
const pollFomcPage = async () => {
    attempt++;
    console.log(`[⏱️ ${attempt}/${MAX_ATTEMPTS}] ${new Date().toLocaleString()} - Fetching...`);
    ...
    try {
        ...
        if (html.includes('<html')) {
            ...
            await conn.query(
                `INSERT INTO fomc_minutes_html (id, html_content) VALUES (?, ?)`,
                [id, html]
            );
...
if (delayMs > 0) {
    console.log(`⏳ ${now.format('YYYY-MM-DD HH:mm:ss')} → ${KST_START_TIME.format('YYYY-MM-DD HH:mm:ss')} 에 시작합니다 (${Math.round(delayMs / 1000)}초 대기)...`);
    setTimeout(() => {
        pollFomcPage();                      // 첫 실행
